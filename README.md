...
- `DETECT_SCREEN_CAPTURE`: only for relocking screenshots after a temporary one-time screenshot permission on Android 14+
...
- `DETECT_SCREEN_CAPTURE`：仅用于 Android 14+ 临时允许一次截屏后重新锁定截屏
...
- `DETECT_SCREEN_CAPTURE`: Android 14+에서 임시 1회 스크린샷 허용 후 다시 잠그는 데만 사용
